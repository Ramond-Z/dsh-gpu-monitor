...
import { probeServer, queryGpus, queryServer, targetHost, closeSshConnections } from "./query.mjs";
...
export function createMonitorEngine(opts = {}) {
  ...
  const settings = {
    ...
    marqueeSpeedPx: 45, // 进程命令行跑马灯滚动速度（px/s）；设置文件里的值优先
  };
   ...
   *  "已知"= 当前可见（本机/可用 server/最近查询结果）∪ ssh config 候选（含探测不可达的）——
   *  机器瞬时不可达时不得从存储顺序里剔除（否则会截断用户保存的排序，恢复后也排回末尾）。
   *  探测尚未完成时直接返回存储顺序原样（此时还不知道当前机器集合，调和只会误伤）。 */
  function serveOrder() {
    if (useSshConfig && !discovered) return orderStore.get();
    const ids = serverIds();
    const known = new Set(ids);
    for (const c of allCandidates) known.add(keyOf(c));
    const { o, t } = orderStore.get();
    const seen = new Set();
    const out = [];
    for (const id of o) if (known.has(id) && !seen.has(id)) { out.push(id); seen.add(id); }
  ...
  async function discover() {
    try {
      ...
      available = [];
      discovered = true;
      // 不更新 allCandidates：保留上次成功解析的候选，避免存储顺序被截断
  ...
  /** 停止所有定时器与订阅，并优雅关闭各 server 的常驻 SSH 主连接。 */
  function stop() {
    started = false;
    if (tickTimer) clearInterval(tickTimer);
    if (discoverTimer) clearInterval(discoverTimer);
  ...
  function getSettings() {
    return {
      ...
      marqueeSpeedPx: settings.marqueeSpeedPx,
      useSshConfig,
    };
  }
