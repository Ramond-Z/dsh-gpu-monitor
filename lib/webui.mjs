...
<head>
  ...
  <style>
    ...
    @media (prefers-color-scheme: light) {
      ...
      html:not([data-gpu-theme="dark"]) .app-bar .refresh { color: rgba(35,39,47,.7); }
      html:not([data-gpu-theme="dark"]) .app-bar .refresh:hover { background: rgba(0,0,0,.07); color: #111; }
    }
    .app-bar {
    ...
    .app-bar .refresh {
      flex: none; width: 24px; height: 24px; margin-left: -6px;
      display: inline-flex; align-items: center; justify-content: center;
      border: none; border-radius: 50%; background: transparent; padding: 0;
      color: rgba(219,224,234,.7); font-size: 15px; line-height: 1; cursor: pointer;
      transition: background .15s, color .15s, transform .15s;
    }
    .app-bar .refresh:hover { background: rgba(255,255,255,.1); color: #fff; }
