...
/** 页面骨架（顶栏状态条；主题跟随系统深浅色——不写死 data-gpu-theme，
 *  client.js 的 syncTheme 会按系统偏好维护它；data-gpu-mode=app 占满窗口、无高度把手）。 */
export const INDEX_HTML = `<!DOCTYPE html>
<html lang="zh-CN" data-gpu-mode="app">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>GPU 监控</title>
  <style>
    html { color-scheme: light dark; } /* 原生控件/滚动条跟随系统 */
    ...
    /* 浅色模式：纯媒体查询随系统切换，必须放在基础（深色）样式之后——
       同特异性下后写的规则胜出，否则浅色覆盖会被深色基础样式压掉 */
    @media (prefers-color-scheme: light) {
      body {
        background: radial-gradient(1100px 700px at 75% -10%, #ffffff 0%, #eef0f5 55%) fixed, #eef0f5;
        color: #23272f;
      }
      .app-bar { border-bottom-color: rgba(0,0,0,.08); }
      .app-bar .meta { color: rgba(35,39,47,.55); }
      .app-bar .refresh { border-color: rgba(0,0,0,.12); background: rgba(0,0,0,.05); color: rgba(35,39,47,.85); }
      .app-bar .refresh:hover { background: rgba(0,0,0,.1); color: #111; }
    }
  </style>
</head>
<body>
  <header class="app-bar">
    ...
    <button type="button" class="refresh" id="btn-settings" title="设置">⚙</button>
  ...
  <script>
    ...
    (function () {
      ...
      // 设置：由 lib/client.js 的监听器打开设置弹窗（同一份 UI，零重复）
      document.getElementById("btn-settings").addEventListener("click", function () {
        window.dispatchEvent(new CustomEvent("gpu-monitor-open-settings"));
      });
    })();
  </script>
</body>
