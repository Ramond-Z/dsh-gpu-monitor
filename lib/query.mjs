...
// —— CPU / 内存采样（Linux /proc）——
// 一次 `cat` 取回 /proc/stat、/proc/meminfo、/proc/loadavg：一条命令、无 shell 字符串，
// 与 nvidia-smi/ps 同一套 spawn 数组参数执行方式（本地直跑，远程经 SSH 复用同一条主连接）。
// CPU% 由相邻两次采样的计数器差算出（/proc/stat 只有累计值）——因此第一次采样只有内存，
// 第二次起（下一轮轮询）才有 CPU 占用。内存可直接由 MemTotal/MemAvailable 算出。
const SYSTEM_FILES = ["/proc/stat", "/proc/meminfo", "/proc/loadavg"];
const MIN_CPU_WINDOW_MS = 500; // 距上次采样太近（如手动刷新紧跟周期查询）时沿用上次读数，避免 0 秒窗口的噪声
const cpuSamples = new Map(); // host -> {total, idle, at, pct, windowSec}（上一轮采样，算差值用）

/** 读取目标机上的 /proc 文件（本地或经 SSH）。 */
function catProc(target, files, timeoutMs) {
  const pre = remoteSshPrefix(target);
  if (!pre) return run("cat", files, timeoutMs);
  return run("ssh", [...pre, "cat", ...files], timeoutMs, () => dropStaleControlSocket(pre));
}
...
/**
 * 解析 `cat /proc/stat /proc/meminfo /proc/loadavg` 的拼接输出（导出以支持单元测试）。
 * @param {string} text
 * @returns {{cpuTicks: {total: number, idle: number}|null, cores: number,
 *            memTotalMB: number, memUsedMB: number, load1: number|null}|null}
 *   无法识别（非 Linux 输出）时返回 null。
 */
export function parseSystemSample(text) {
  ...
  let memAvailKB = null;
  let memBufKB = null;
  let memCachedKB = null;
  for (const line of text.split("\n")) {
    if (line.startsWith("cpu")) {
      const m = /^cpu(\d*)\s+([\d\s]+)$/.exec(line.trim());
      if (!m) continue;
      if (m[1]) { cores++; continue; } // cpu0/cpu1/… 每核一行 → 核数
      const f = m[2].trim().split(/\s+/).map(Number);
      if (f.length < 4 || f.some((v) => !Number.isFinite(v))) continue;
      // 取 user nice system idle iowait irq softirq steal 八项；guest/guest_nice 已含在 user/nice 中，不重复计
      const g = f.slice(0, 8);
      const total = g.reduce((a, b) => a + b, 0);
      const idle = g[3] + (g[4] || 0); // 空闲 = idle + iowait
      cpuTicks = { total, idle };
      continue;
    }
    const kv = /^(MemTotal|MemFree|MemAvailable|Buffers|Cached):\s+(\d+)\s*kB/.exec(line);
    if (kv) {
      const v = Number(kv[2]);
      if (kv[1] === "MemTotal") memTotalKB = v;
      else if (kv[1] === "MemFree") memFreeKB = v;
      else if (kv[1] === "MemAvailable") memAvailKB = v;
      else if (kv[1] === "Buffers") memBufKB = v;
      else memCachedKB = v;
      continue;
    }
    if (load1 === null && /^\d+\.\d+\s+\d+\.\d+\s+\d+\.\d+\s+\d+\/\d+/.test(line.trim())) {
      load1 = Number(line.trim().split(/\s+/)[0]);
    }
  }
  // 空闲内存：优先 MemAvailable（内核 ≥3.14）；老内核退回 MemFree+Buffers+Cached
  const availKB = memAvailKB !== null
    ? memAvailKB
    : (memFreeKB || 0) + (memBufKB || 0) + (memCachedKB || 0);
  const memTotalMB = memTotalKB !== null ? Math.round(memTotalKB / 1024) : 0;
  const memUsedMB = memTotalKB !== null ? Math.max(0, memTotalMB - Math.round(availKB / 1024)) : 0;
  if (cpuTicks === null && memTotalMB === 0) return null;
  return { cpuTicks, cores, memTotalMB, memUsedMB, load1 };
}

/**
 * 由相邻两次 /proc/stat 采样算 CPU 占用（%）：busy 差值 / 总差值。
 * @param {{total: number, idle: number}|null} prev 上一次采样
 * @param {{total: number, idle: number}|null} cur 本次采样
 * @returns {number|null} 差值不可用（首次采样 / 计数器未前进）时为 null
 */
export function cpuUsagePct(prev, cur) {
  if (!prev || !cur) return null;
  const dTotal = cur.total - prev.total;
  const dIdle = cur.idle - prev.idle;
  if (!(dTotal > 0)) return null;
  return pctOf(dTotal - dIdle, dTotal);
}

/**
 * 查询一台机器的 CPU / 内存占用（本机或经 SSH，仅 Linux；失败返回 null，不影响 GPU 查询）。
 * CPU% 是「距上次采样区间」的平均值（轮询间隔即采样间隔），故首次采样只有内存没有 CPU；
 * 两次采样间隔不足 MIN_CPU_WINDOW_MS 时沿用上次读数（手动刷新紧跟周期查询时不产生 0 秒窗口噪声）。
 * @param {null|string|object} target 见 targetArgs
 * @param {number} [timeoutMs]
 * @returns {Promise<{cpu: {pct: number|null, cores: number, load1: number|null, windowSec: number|null},
 *                    mem: {usedMB: number, totalMB: number, pct: number}|null}|null>}
 */
export async function querySystem(target, timeoutMs = 8000) {
  try {
    const r = await catProc(target, SYSTEM_FILES, timeoutMs);
    if (r.code !== 0) return null;
    const s = parseSystemSample(r.out);
    if (!s) return null;
    const key = targetHost(target);
    const now = Date.now();
    const prev = cpuSamples.get(key) || null;
    let sample = prev; // 默认沿用上次（含无 CPU 计数器时的情形）
    if (s.cpuTicks && (!prev || now - prev.at >= MIN_CPU_WINDOW_MS)) {
      sample = {
        ...s.cpuTicks,
        at: now,
        pct: cpuUsagePct(prev, s.cpuTicks), // 首个窗口为 null（只有累计值，无差值）
        windowSec: prev ? Math.round((now - prev.at) / 100) / 10 : null,
      };
      cpuSamples.set(key, sample);
    }
    return {
      cpu: {
        pct: sample ? sample.pct : null,
        cores: s.cores,
        load1: s.load1,
        windowSec: sample ? sample.windowSec : null,
      },
      mem: s.memTotalMB > 0
        ? { usedMB: s.memUsedMB, totalMB: s.memTotalMB, pct: pctOf(s.memUsedMB, s.memTotalMB) }
        : null,
    };
  } catch (e) {
    return null; // 采样失败不致命（非 Linux / 无权限 / 超时）
  }
}
...
/**
 * 查询一台机器（本机或经 SSH）的 GPU 用量、计算进程与 CPU/内存占用。
 * GPU 与系统采样并行发起；系统采样失败（非 Linux / /proc 不可读）只缺 cpu/mem 字段，不影响 GPU 结果。
 * @param {null|string|object} target 见 targetArgs
 * @param {{timeoutMs?: number, label?: string}} opts
 * @returns {{host: string, label: string, ok: boolean, at: string, gpus?: object[], error?: string,
 *            cpu?: object, mem?: object}} cpu = {pct, cores, load1, windowSec}；mem = {usedMB, totalMB, pct}
 */
export async function queryServer(target, opts = {}) {
  ...
  const [r, sys] = await Promise.all([
    smi(target, [`--query-gpu=${QUERY}`, "--format=csv,noheader,nounits"], timeoutMs),
    querySystem(target, timeoutMs),
  ]);
  const sysFields = sys ? { cpu: sys.cpu, mem: sys.mem } : {};
  if (r.code !== 0) {
  ...
  if (gpus.length === 0)
    return { host, label: label || host, ok: false, at, error: "nvidia-smi 无输出（无 GPU？）", ...sysFields };
  // 每卡进程（失败不致命，进程字段为空数组）
  const procs = await queryProcesses(target, timeoutMs);
  for (const g of gpus) {
    g.processes = procs && procs.procsByIndex[g.index] ? procs.procsByIndex[g.index] : [];
  }
  return { host, label: label || host, ok: true, at, gpus, ...sysFields };
}
