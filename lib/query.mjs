...
/**
 * 查询 GPU 用量。
 * @param {{sshTarget?: string, timeoutMs?: number, servers?: Array<null|string|object>,
 *          includeLocal?: boolean, localLabel?: string}} opts
 *   - 传 `servers`（非空数组）→ 多机模式：并行查询每台（可含本机），返回
 *     {ok, at, source:"host", servers:[...]}。
 *   - 否则 → 兼容旧调用（本机或 sshTarget 单目标），返回扁平 {ok, at, gpus?, error?}。
 * @returns {Promise<object>}
 */
export async function queryGpus(opts = {}) {
  const { sshTarget = "", timeoutMs = 8000, servers, includeLocal = true, localLabel = "本机" } = opts;
  ...
  // 兼容旧调用：单目标扁平结构
  const r = await queryServer(sshTarget || null, { timeoutMs, label: sshTarget || "本机" });
  return { ok: r.ok, at: r.at, gpus: r.gpus, error: r.error };
}
