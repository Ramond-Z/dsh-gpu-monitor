// dsh-gpu-monitor: 可复用 HTTP 传输层 —— CLI sidecar 与 Electron 原生应用共用。
// 路由：状态 /status、/gpu-status.json、/gpu/status；顺序 POST /order；设置 GET/POST /settings；
// 独立网页 UI（/、/dsh-shim.js、client.js）。
import { createServer } from "node:http";
import { readFileSync } from "node:fs";
import { INDEX_HTML, SHIM_JS, CLIENT_JS_PATH } from "./webui.mjs";

const CORS_HEADERS = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
  "Access-Control-Allow-Headers": "Content-Type",
};

const json = (res, status, body) => {
  res.writeHead(status, {
    ...CORS_HEADERS,
    "Content-Type": "application/json; charset=utf-8",
    "Cache-Control": "no-store",
  });
  res.end(JSON.stringify(body));
};

/** 读取请求体（限制大小，防滥用）。 */
function readBody(req, limit = 64 * 1024) {
  return new Promise((resolve, reject) => {
    let size = 0;
    const chunks = [];
    req.on("data", (c) => {
      size += c.length;
      if (size > limit) { reject(new Error("body too large")); req.destroy(); return; }
      chunks.push(c);
    });
    req.on("end", () => resolve(Buffer.concat(chunks).toString("utf8")));
    req.on("error", reject);
  });
}

/**
 * 状态快照处理器（/status、/gpu-status.json、/gpu/status 共用；宿主插件也注册同一个处理器，
 * 保证 DSH 宿主与 sidecar/Electron 输出同一份线格式，头信息不重复实现）。
 * @param {object} engine 共享监控引擎
 * @returns {(req: object, res: object) => void}
 */
export function makeStateHandler(engine) {
  return (req, res) => json(res, 200, engine.getState());
}

/**
 * 分组顺序处理器（POST /order 共用；宿主插件也注册同一个处理器）。
 * 接收 {o: string[], t: number}，引擎校验时间戳、持久化并返回调和后的顺序。
 * @param {object} engine 共享监控引擎
 * @returns {(req: object, res: object) => Promise<void>}
 */
export function makeOrderHandler(engine) {
  return async (req, res) => {
    if (req.method !== "POST") {
      res.writeHead(405, CORS_HEADERS);
      res.end();
      return;
    }
    try {
      const body = JSON.parse(await readBody(req));
      const next = engine.setOrder(body?.o, Number(body?.t));
      if (next === null) {
        json(res, 400, { ok: false, error: "需要 {o: string[], t: number}" });
        return;
      }
      json(res, 200, { ok: true, order: next });
    } catch (e) {
      json(res, 400, { ok: false, error: String(e).replace(/^Error:\s*/, "") });
    }
  };
}

/**
 * 设置处理器（GET /settings、POST /settings 共用；宿主插件也注册同一个处理器）。
 * GET：当前生效设置 + 候选 server 列表（首次打开时若尚未探测会触发一次探测）；
 * POST：应用设置补丁（引擎校验 → 持久化 → 立即生效）。
 * @param {object} engine 共享监控引擎
 * @returns {(req: object, res: object) => Promise<void>}
 */
export function makeSettingsHandler(engine) {
  return async (req, res) => {
    if (req.method === "POST") {
      try {
        const body = JSON.parse(await readBody(req));
        const out = engine.setSettings(body);
        json(res, 200, { ok: true, settings: out.settings, candidates: out.candidates });
        return;
      } catch (e) {
        json(res, 400, { ok: false, error: String(e).replace(/^Error:\s*/, "") });
        return;
      }
    }
    try {
      await engine.ensureDiscovered();
    } catch (e) {
      /* 探测失败不阻塞设置页（候选列表可能为空） */
    }
    json(res, 200, {
      ok: true,
      settings: engine.getSettings(),
      candidates: engine.serverCandidates(),
    });
  };
}

/**
 * 启动监控 HTTP 服务。
 * @param {object} opts
 * @param {object} opts.engine 共享监控引擎（lib/engine.mjs）
 * @param {string} [opts.host] 监听地址（默认 127.0.0.1）
 * @param {number} [opts.port] 监听端口（默认 0 = 随机）
 * @param {boolean} [opts.serveUi] 是否提供独立网页 UI（默认 true）
 * @param {(...a: any[]) => void} [opts.log]
 * @returns {Promise<{port: number, close: () => Promise<void>}>}
 */
export async function createMonitorServer(opts = {}) {
  const { engine, host = "127.0.0.1", port = 0, serveUi = true, log = () => {} } = opts;
  const stateHandler = makeStateHandler(engine);
  const settingsHandler = makeSettingsHandler(engine);
  const orderHandler = makeOrderHandler(engine);
  let actualPort = port; // 监听完成后更新为真实端口（port=0 时为随机端口）
  const server = createServer(async (req, res) => {
    const url = new URL(req.url || "/", `http://${host}:${port}`);
    if (req.method === "OPTIONS") {
      res.writeHead(204, CORS_HEADERS);
      res.end();
      return;
    }
    // —— 独立网页 UI（MacBook / Electron / 无 DSH 环境） ——
    if (serveUi && (req.method === "GET" || req.method === "HEAD")) {
      if (url.pathname === "/") {
        res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
        res.end(INDEX_HTML);
        return;
      }
      if (url.pathname === "/dsh-shim.js") {
        res.writeHead(200, { "Content-Type": "text/javascript; charset=utf-8", "Cache-Control": "no-store" });
        res.end(SHIM_JS);
        return;
      }
      if (url.pathname === "/plugins/dsh-gpu-monitor/client.js") {
        try {
          res.writeHead(200, { "Content-Type": "text/javascript; charset=utf-8", "Cache-Control": "no-store" });
          res.end(readFileSync(CLIENT_JS_PATH));
          return;
        } catch {
          res.writeHead(404);
          res.end("client.js not found");
          return;
        }
      }
    }
    // —— 数据接口 ——
    if (url.pathname === "/status" || url.pathname === "/gpu-status.json" || url.pathname === "/gpu/status") {
      stateHandler(req, res);
      return;
    }
    if (url.pathname === "/settings") {
      await settingsHandler(req, res);
      return;
    }
    if (url.pathname === "/order" && req.method === "POST") {
      await orderHandler(req, res);
      return;
    }
    // 手动刷新：立即触发一次引擎查询，引擎保证返回查询完成后的新状态快照
    if (url.pathname === "/refresh" && (req.method === "POST" || req.method === "GET")) {
      try {
        json(res, 200, await engine.refresh());
      } catch (e) {
        json(res, 500, { ok: false, error: String(e) });
      }
      return;
    }
    if (url.pathname === "/health") {
      json(res, 200, { ok: true, servers: engine.listServers(), port: actualPort });
      return;
    }
    res.writeHead(404, CORS_HEADERS);
    res.end("not found");
  });

  await new Promise((resolve, reject) => {
    server.once("error", reject);
    server.listen(port, host, resolve);
  });

  actualPort = server.address().port;
  return {
    port: actualPort,
    close: () => new Promise((resolve) => server.close(resolve)),
  };
}
