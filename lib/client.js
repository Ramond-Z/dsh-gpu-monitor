  ...
  factory: (require) => {
    ...
    function injectTheme() {
      ...
      s.textContent =
        ...
        // flex-basis = "4 个方块的精确宽度"（42px×4 + 3×4px 间隙 = 180px）—— flex 只有在放得下
        ...
        ".dsh-gpu-monitor .gpu-grid{display:flex;flex-wrap:wrap;gap:4px;flex:1 1 180px;min-width:0;align-content:flex-start}" +
        // CPU/内存：两条竖向细条（填充高度=占用率、颜色=与方块同一套分级），随方块区高度拉伸。
        // 百分数放在条**下方横排**：列宽 18px 是按最宽的 `100%` 定的（7px 字号约 17.5px）——
        // 列窄了两位数也会溢出到相邻列，看起来就是两个数字叠在一起。
        // align-self:stretch 让细条与方块网格同高；track 的 min-height 兜底：高度链万一算不出来也不会被压成 0。
        ".dsh-gpu-monitor .gpu-sys{display:flex;gap:3px;flex:0 0 auto;align-self:stretch;min-height:42px}" +
        ".dsh-gpu-monitor .gpu-sys-col{display:flex;flex-direction:column;align-items:center;gap:2px;width:18px;flex:none}" +
        ".dsh-gpu-monitor .gpu-sys-tag{font-size:7px;line-height:1;color:var(--gpu-fg-dim);flex:none}" +
        ".dsh-gpu-monitor .gpu-sys-track{box-sizing:border-box;position:relative;flex:1;width:18px;min-height:26px;" +
        ...
        ".dsh-gpu-monitor .gpu-sys-val{flex:none;font-size:7px;font-weight:700;line-height:1;color:var(--gpu-fg);white-space:nowrap;" +
    ...
    function makeBlock(serverId, index) {
      const block = el("div", {
        className: "gpu-block",
        style:
          // box-sizing:border-box：宽度**含边框**，才能用"42px×4 + 间隙 = 180px"精确算出
          // 网格的 flex-basis（content-box 下 1px 边框会让每块多占 2px，4 块一行就放不下了）
          "box-sizing:border-box;position:relative;width:42px;height:42px;border-radius:8px;overflow:hidden;" +
