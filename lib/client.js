...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function positionDropMarker(y) {
      ...
      else if (lastEl) top = lastEl.getBoundingClientRect().bottom - gRect.top + groupsEl.scrollTop - 2;
      else top = 0;
    ...
    function commitDrag() {
      ...
      // Last：执行移动
      groupsEl.insertBefore(group, before || errorBannerEl);
      persistOrderFromDom();
      // 复位拖拽样式
      group.style.opacity = "";
      group.style.outline = "";
    ...
    function initGroupDrag(group, head, s) {
      head.addEventListener("pointerdown", (e) => {
        ...
        const onMove = (ev) => {
          ...
          // 接近容器上下边缘时自动滚动
          const gRect = groupsEl.getBoundingClientRect();
          if (ev.clientY < gRect.top + 28) groupsEl.scrollTop -= 10;
          else if (ev.clientY > gRect.bottom - 28) groupsEl.scrollTop += 10;
        };
        const onUp = () => {
