...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 尽力把顺序同步给 sidecar（跨浏览器共享）。失败静默（本浏览器 localStorage 仍生效）。
     * 候选按序尝试：显式覆盖 → 当前页面同源（独立网页=sidecar 自己）→ DSH 默认 127.0.0.1:3499。 */
    function postOrder(o, t) {
      const candidates = [];
      const add = (u) => { try { const x = new URL(u, location.origin).origin + "/order"; if (!candidates.includes(x)) candidates.push(x); } catch {} };
      try {
      ...
      const attempt = (i) => {
        ...
        fetch(candidates[i], {
          ...
          .then((res) => { if (!res.ok) attempt(i + 1); })
          .catch(() => attempt(i + 1));
      };
      attempt(0);
    }
    ...
    function maybeAdoptServerOrder(d, ids) {
      ...
      if (chosen && ts > orderSyncTs) {
        serverOrder = reconcileOrder(chosen, ids);
        serverOrderTs = ts;
        orderSyncTs = ts;
    ...
    /** 主题跟随：优先页面显式 data-gpu-theme；其次 DSH 在 <html> 上写的内联 color-scheme；最后系统偏好。 */
    function syncTheme() {
      const de = document.documentElement;
      if (de.dataset.gpuTheme === "dark" || de.dataset.gpuTheme === "light") return; // 页面已显式指定（独立页/应用）
      const inline = de.style && de.style.colorScheme;
      if (inline === "dark" || inline === "light") { de.dataset.gpuTheme = inline; return; }
      de.dataset.gpuTheme = matchMedia && matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
    }
    ...
    function injectTheme() {
      ...
      s.textContent =
        ".dsh-gpu-monitor{" + darkVars + "}" +
        'html[data-gpu-theme="light"] .dsh-gpu-monitor{' + lightVars + "}" +
        "@media (prefers-color-scheme: light){.dsh-gpu-monitor{" + lightVars + "}}" +
        'html[data-gpu-theme="dark"] .dsh-gpu-monitor{' + darkVars + "}" +
    ...
    /** 从当前 DOM 顺序持久化 serverOrder（排除错误/警告横幅与指示线）；写入 localStorage 并回同步 sidecar。 */
    ...
    function renderGroup(g, s, isFirst) {
      ...
      g.time.title = "";
      // 表头：x/y 张 GPU（x = 空闲数，y = 总数）。空闲 = 利用率 <5% 且显存占用 <1G 且无计算进程
      let idle = 0;
      for (const gg of s.gpus) {
        const util = Number(gg.utilPct) || 0;
        const mem = Number(gg.memUsedMB) || 0;
        const procs = Array.isArray(gg.processes) ? gg.processes.length : 0;
        if (util < 5 && mem < 1024 && procs === 0) idle++;
      }
      const total = s.gpus.length;
      const text = total > 0 ? `${idle}/${total} 张 GPU` : "";
      g.title.textContent = `🎮 ${s.label}${text ? " · " + text : ""}`;
      g.title.title = total > 0 ? `${idle} 张空闲 / ${total} 张总数` : text;
      for (const gg of s.gpus) {
    ...
    async function tick() {
      ...
      if (d) {
        ...
        maybeAdoptServerOrder(d, lastGood.servers.map((s) => s.id));
      }
