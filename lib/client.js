...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SIDECAR_URL = "http://127.0.0.1:3499/status";
    ...
    function buildMonitorInto(root, isWide) {
      ...
      groupsEl = el("div", {
        style:
          "display:flex;flex-direction:column;gap:2px;max-height:42vh;overflow-y:auto;" +
    ...
    /** sidecar 地址：window.__DSH_GPU_MONITOR__.sidecarUrl 或 ?gpuMonitorSidecar= 覆盖；空串=禁用。 */
    ...
    async function tick() {
      let d = null;
      // 优先 sidecar（多机）；失败回退同源 /gpu/status
      const sidecarUrl = sidecarUrlOf();
      if (sidecarUrl) {
        try {
          d = await fetchJson(sidecarUrl, SIDECAR_TIMEOUT_MS);
        } catch (e) {
          d = null;
        }
      }
      if (!d || !Array.isArray(d.servers)) {
        try {
          d = await fetchJson(STATUS_URL, FETCH_TIMEOUT_MS);
        } catch (e) {
          render({ ok: false, error: `连接失败 ${sidecarUrl ? sidecarUrl + " / " : ""}${STATUS_URL}\n${e && e.message ? e.message : e}` });
