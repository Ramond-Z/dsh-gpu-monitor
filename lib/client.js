...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 从当前 DOM 顺序持久化 serverOrder（排除错误/警告横幅）。 */
    ...
    function initGroupDrag(group, head, s) {
      const begin = (e) => {
        ...
        const onMove = (ev) => {
          ...
          // 在其它分组中间找插入点（按当前 DOM 顺序）
          const y = ev.clientY;
          let before = null;
          for (const child of groupsEl.children) {
            const other = [...groupNodes.entries()].find(([, x]) => x.group === child);
            if (!other || other[0] === s.id) continue;
            const r = child.getBoundingClientRect();
            if (y < r.top + r.height / 2) { before = child; break; }
          }
          if (before) groupsEl.insertBefore(group, before);
          else groupsEl.insertBefore(group, errorBannerEl); // 没有更靠下的分组 → 排到（横幅前）最后
          persistOrderFromDom();
        };
        const end = () => {
      ...
      head.addEventListener("pointerdown", begin);
    }
