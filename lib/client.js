...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      try {
        bridge.onFallback(() => {
          ...
          if (hoverServerId != null) {
            const blk = hoverBlock();
            if (blk) positionTip(blk.getBoundingClientRect());
          }
          if (tip) {
    ...
    function renderGroup(g, s, isFirst) {
      ...
      const errs = g.blocksEl.querySelectorAll(".gpu-error");
    ...
    function renderTip() {
      if (hoverServerId == null || hoverGpuIndex == null || !tip) return 0;
