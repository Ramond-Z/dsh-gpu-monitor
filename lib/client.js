  ...
  factory: (require) => {
    ...
    function injectTheme() {
      ...
      s.textContent =
        ...
        // flex-basis = "4 个方块的精确宽度"（45px×4 + 3×4px 间隙 = 192px）—— flex 只有在放得下
        // "4 个方块 + 细条" 时才让两者同行，否则细条整块换到下一行：两种情况方块都是 4 个一行。
        // （方块/细条都设了 box-sizing:border-box，宽度含边框，这里的算式才成立。）
        ".dsh-gpu-monitor .gpu-blocks{display:flex;flex-wrap:wrap;gap:4px;padding:8px 6px;align-items:flex-start;justify-content:flex-end}" +
        ".dsh-gpu-monitor .gpu-grid{display:flex;flex-wrap:wrap;gap:4px;flex:1 1 192px;min-width:0;align-content:flex-start}" +
        // CPU/内存：两条竖向细条（填充高度=占用率、颜色=与方块同一套分级），随方块区高度拉伸
        // align-self:stretch 覆盖 .gpu-blocks 的 flex-start，让细条与方块网格同高；
        // track 的 min-height 是兜底：万一 flex 高度链没算出高度，细条也必须可见（不能被压成 0）
        ".dsh-gpu-monitor .gpu-sys{display:flex;gap:2px;flex:0 0 auto;align-self:stretch;min-height:45px}" +
        ".dsh-gpu-monitor .gpu-sys-col{display:flex;flex-direction:column;align-items:center;gap:2px}" +
        ".dsh-gpu-monitor .gpu-sys-tag{font-size:7px;line-height:1;color:var(--gpu-fg-dim);flex:none}" +
        ".dsh-gpu-monitor .gpu-sys-track{box-sizing:border-box;position:relative;flex:1;width:10px;min-height:30px;" +
        "border-radius:5px;overflow:hidden;background:var(--gpu-track);border:1px solid var(--gpu-border)}" +
        ...
        ".dsh-gpu-monitor .gpu-sys-val{position:absolute;left:0;right:0;top:2px;text-align:center;font-size:8px;font-weight:700;" +
    ...
    /** 系统细条（CPU / 内存各一条）：轨道 + 自底向上填充 + 竖排百分数。 */
    function makeSysBar(tag, name) {
      ...
      const val = el("div", { className: "gpu-sys-val", textContent: "—" }, track);
      return { col, track, fill, val, name };
    }

    /** 更新一条系统细条：占用率 → 填充高度 + 分级色 + 竖排百分数（无数据时显示 —）。 */
    ...
    function makeBlock(serverId, index) {
      const block = el("div", {
        className: "gpu-block",
        style:
          // box-sizing:border-box：宽度**含边框**，才能用"45px×4 + 间隙 = 192px"精确算出
          // 网格的 flex-basis（content-box 下 1px 边框会让每块多占 2px，4 块一行就放不下了）
          "box-sizing:border-box;position:relative;width:45px;height:45px;border-radius:8px;overflow:hidden;" +
    ...
    function renderSysTip() {
      ...
      if (cpu) {
        ...
        const win = cpu.pct != null && cpu.windowSec ? `（近 ${cpu.windowSec}s 平均）` : "";
        el("div", { style: "margin-top:2px" }, tip).textContent = `CPU ${pct}${win}`;
        const bits = [];
