...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // —— Electron 悬浮窗桥（tip-preload.cjs 暴露）：进程提示额外渲染到独立透明置顶小窗，
    //    可伸出面板窗口范围（面板 DOM 会被窗口边界裁切）。页面内提示**始终显示**作为保底：
    //    悬浮窗与页面内提示锚定同一位置、层级更高，可用时正好盖住页面内提示并伸出窗口；
    //    不可用时页面内提示照常可见——任何环境下悬停都有提示。 ——
    ...
    function showTip(serverId, index, block) {
      ...
      ensureTip();
      renderTip();
      // Electron：同时推给独立悬浮窗（盖在页面内提示上方、可伸出面板窗口）。
      // 页面内提示照常显示——悬浮窗不可用时它就是保底。
      if (tipBridge) {
        try {
