...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 从完整命令行提炼简短标签：剥解释器前缀，脚本取文件名，最多带 2 个关键参数。 */
    function shortCmd(cmd) {
      if (!cmd) return "";
      let s = cmd.trim().replace(
        /^(?:\S+\/)*(?:uv\s+run\s+|conda\s+run\s+|nohup\s+)*(?:python3?(?:\.[0-9]+)?|bash|sh)\s+/i,
        ""
      );
      const parts = s.split(/\s+/).filter(Boolean);
      if (!parts.length) return cmd.trim().split("/").pop().slice(0, 40);
      const out = [];
      for (let i = 0; i < parts.length && out.length < 3; i++) {
        const v = i === 0 ? parts[i].split("/").pop() : parts[i];
        if (v.length > 40) continue;
        out.push(v);
      }
      return out.length ? out.join(" ") : cmd.trim().split("/").pop().slice(0, 40);
    }
    ...
    function renderBlocks() {
      ...
      if (headTitle) {
        ...
        for (const g of data.gpus) {
          const n = (g.name || "GPU").replace(/^NVIDIA\s+/i, "").replace(/^GeForce\s+/i, "");
    ...
    function renderTip() {
      ...
      // 内容没变化就不重建（避免打断选中/复制）
      const sig = g
        ? JSON.stringify([hoverIdx, g.memUsedMB, g.memTotalMB, g.utilPct, g.tempC, g.powerW, g.processes])
        : "none";
      ...
      for (const p of procs) {
        ...
        el("div", {
          ...
          textContent: `● ${p.user || "?"} · ${p.pid} · ${fmtMem(p.memMB)} · ${label}`,
          title: p.cmd || "",
        }, tip);
      }
    }
