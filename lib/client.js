...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // —— 细条宽度档位（运行时按实测可用宽度选档） ——
    // 目标：面板不够宽时**优先收窄细条**，而不是让细条换行到方块下方（方块每行始终 4 个）。
    // need = 方块宽×4 + 方块间隙×3 + 细条与网格间隙 + 细条宽；每项必须与 CSS 一致：
    //   方块宽（.gpu-block 的 --gpu-block-size 默认值，border-box）
    //   方块间隙 BLOCK_GAP = .gpu-grid 的 gap；细条与网格间隙 ROW_GAP = .gpu-blocks 的 gap
    //   细条宽 = 两列 colW + 列间隙 gap（.gpu-sys-col / .gpu-sys-track 的 width、.gpu-sys 的 gap）
    const BLOCK_GAP = 4; // .gpu-grid 的 gap
    const ROW_GAP = 3; // .gpu-blocks 的 gap（网格与细条之间）
    const RAIL_RUNGS = [
      // 宽档（= CSS 默认值）：44px 方块 + 带条下百分数的细条 → need = 44×4+4×3+3+31 = 222
      { name: "wide", bw: 44, colW: 14, gap: 3, labels: true },
      // 窄档：方块不动，细条收窄并收起百分数（悬停仍可看） → need = 213
      { name: "narrow", bw: 44, colW: 10, gap: 2, labels: false },
      // 小方块档：细条已最窄还不够时把方块缩到 40px，保住"4 个一行 + 细条在右" → need = 197
      { name: "small", bw: 40, colW: 10, gap: 2, labels: false },
    ];
    const BLOCK_W_DEFAULT = RAIL_RUNGS[0].bw; // 与 CSS 的 --gpu-block-size 默认值一致
    const GRID_BASIS_DEFAULT = BLOCK_W_DEFAULT * 4 + BLOCK_GAP * 3; // 与 CSS 的 --gpu-grid-basis 默认值一致

    /** 某档位的细条宽度（两列 + 列间隙）。 */
    function railWidth(r) {
      return r.colW * 2 + r.gap;
    }

    /** 某档位需要的可用宽度（.gpu-blocks 的内容宽度）。 */
    function railNeed(r) {
      return r.bw * 4 + BLOCK_GAP * 3 + ROW_GAP + railWidth(r);
    }

    /** 选档：从宽到窄取第一个放得下的；都放不下就回宽档（此时细条只能换行，至少数字还看得见）。 */
    function pickRailRung(avail) {
      if (avail > 0) for (const r of RAIL_RUNGS) if (avail >= railNeed(r)) return r;
      return RAIL_RUNGS[0];
    }

    /** 应用档位：方块尺寸与网格 flex-basis 走 CSS 变量（整组一次生效，不必逐块改）。 */
    function applyRailRung(g, rung) {
      g.group.style.setProperty("--gpu-block-size", rung.bw + "px");
      g.group.style.setProperty("--gpu-grid-basis", rung.bw * 4 + BLOCK_GAP * 3 + "px");
      for (const bar of [g.cpu, g.mem]) {
        bar.col.style.width = rung.colW + "px";
        bar.track.style.width = rung.colW + "px";
        bar.val.style.display = rung.labels ? "" : "none"; // 窄档收起百分数（悬停看明细）
      }
      g.sysEl.style.gap = rung.gap + "px";
    }
    ...
    function injectTheme() {
      ...
      s.textContent =
        ...
        ".dsh-gpu-monitor .gpu-grid{display:flex;flex-wrap:wrap;gap:4px;flex:1 1 var(--gpu-grid-basis,188px);min-width:0;align-content:flex-start}" +
    ...
    function makeGroup(s) {
      ...
      // 细条**不设 title 属性**：原生 tooltip 会和我们的提示盒同时弹出（悬停时出现"一大一小"
      // 两个框），细条的信息全在提示盒里（鼠标悬停 / 触摸点按都会触发）。
      const sysEl = el("div", { className: "gpu-sys" }, blocksEl);
     ...
     *  不给这些元素设 title —— 原生 tooltip 会与提示盒重复显示。 */
    function makeSysBar(tag, name) {
      const col = el("div", { className: "gpu-sys-col" });
      el("div", { className: "gpu-sys-tag", textContent: tag }, col);
      const track = el("div", { className: "gpu-sys-track" }, col);
     ...
     *  顺带按实测可用宽度选细条档位：宁可细条收窄（甚至收起百分数），也不让它换行到方块下方。 */
    function renderSystem(g, s) {
      ...
      if (!has) return;
      // clientWidth 含左右内边距（.gpu-blocks 的 padding: 8px 5px → 各 5px），减掉才是可用宽度
      const rung = pickRailRung(g.blocksEl.clientWidth - 10);
      if (g.railRung !== rung.name) {
        g.railRung = rung.name;
        applyRailRung(g, rung);
      }
      setSysBar(g.cpu, cpu && cpu.pct != null ? cpu.pct : NaN);
      setSysBar(g.mem, mem && mem.pct != null ? mem.pct : NaN);
    }
    ...
    function makeBlock(serverId, index) {
      const block = el("div", {
        className: "gpu-block",
        style:
          // box-sizing:border-box：宽度**含边框**，尺寸才是精确可算的（网格 flex-basis 依赖它）。
          // 宽高走 CSS 变量：窄面板时由 applyRailRung 统一改小（默认 44px，见 RAIL_RUNGS[0].bw）
          "box-sizing:border-box;position:relative;width:var(--gpu-block-size,44px);height:var(--gpu-block-size,44px);" +
    ...
    /** 渲染 CPU/内存细条的提示内容（悬停整组细条）：CPU 一行（占用 + 核数 + 负载），内存一行。
    ...
    function renderSysTip() {
      ...
      if (cpu) {
        // 与内存同款：**百分比在前，括号里是 已用/总量**。CPU 没有"已用量"，用 load/核数
        // 充当同一位置的 分子/分母（都是"当前压力 / 容量"），两行读起来是一个格式。
        const pct = cpu.pct == null ? "采样中…" : cpu.pct.toFixed(1) + "%";
        const hasCores = Number.isFinite(cpu.cores) && cpu.cores > 0;
        const hasLoad = Number.isFinite(cpu.load1);
        const detail = hasLoad && hasCores
          ? `load ${cpu.load1} / ${cpu.cores} 核`
          : hasLoad ? `load ${cpu.load1}`
          : hasCores ? `${cpu.cores} 核` : "";
        el("div", { style: "margin-top:2px" }, tip).textContent =
          detail ? `CPU ${pct}（${detail}）` : `CPU ${pct}`;
      }
      if (mem) {
        el("div", { style: "margin-top:2px" }, tip).textContent =
          `内存 ${mem.pct.toFixed(1)}%（${fmtMem(mem.usedMB)} / ${fmtMem(mem.totalMB)}）`;
      }
      return 2;
    }
