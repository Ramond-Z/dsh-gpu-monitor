...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    //    悬浮窗首次渲染有 ack 确认；窗口管线故障时退回页面内提示（保证悬停始终有提示）。 ——
    const tipBridge = window.__gpuMonitorTip;
    let tipWinAcked = false; // 悬浮窗已确认渲染可用
    let tipFallback = false; // 悬浮窗不可用：退回页面内固定提示
    try {
      if (tipBridge && tipBridge.onAck) {
        tipBridge.onAck(() => { tipWinAcked = true; tipFallback = false; }); // ack 迟到=窗口其实可用，解除回退
      }
    } catch (e) {}
    ...
    function showTip(serverId, index, block) {
      ...
      ensureTip();
      renderTip();
      // Electron：提示渲染到独立悬浮窗（可伸出面板窗口范围）；悬浮窗未确认可用前
      // 启动回退计时——窗口管线故障（创建/渲染失败）时退回页面内提示
      if (tipBridge && !tipFallback) {
        try {
          ...
          if (!tipWinAcked) {
            setTimeout(() => {
              if (tipWinAcked) return;
              tipFallback = true; // 悬浮窗不可用：本次悬停立即退回页面内提示
              if (hoverServerId != null) {
                const b = hoverBlock();
                if (b && tip) { positionTip(b.getBoundingClientRect()); tip.style.display = "block"; }
              }
            }, 1500);
          }
          return;
    ...
    function hideTip() {
      ...
      if (tip) tip.style.display = "none"; // 兜底：页面内提示（若有）也隐藏
    }
