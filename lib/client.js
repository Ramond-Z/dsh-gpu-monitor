  ...
  factory: (require) => {
    ...
    function injectTheme() {
      ...
      s.textContent =
        ...
        // flex-basis=4 个方块（44px×4+间隙）保证：够宽时细条并排在方块右侧，不够宽时细条换到
        // 方块下方——方块每行数量在两种情况下都不变（不会因为多了一列而挤掉一个方块）。
        ".dsh-gpu-monitor .gpu-blocks{display:flex;flex-wrap:wrap;gap:6px;padding:8px;align-items:flex-start;justify-content:flex-end}" +
        ...
        ".dsh-gpu-monitor .gpu-sys{display:flex;gap:4px;flex:0 0 auto;align-self:stretch;min-height:44px}" +
        ...
        ".dsh-gpu-monitor .gpu-sys-track{position:relative;flex:1;width:10px;min-height:30px;border-radius:5px;overflow:hidden;" +
        "background:var(--gpu-track);border:1px solid var(--gpu-border)}" +
    ...
    function makeBlock(serverId, index) {
      const block = el("div", {
        className: "gpu-block",
        style:
          // 44px（原 46px）：为右侧 CPU/内存细条让出宽度，252px 面板下仍是 4×2 两排放下 8 卡
