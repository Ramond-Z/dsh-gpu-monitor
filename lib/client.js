...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function renderSysTip() {
      ...
      if (cpu) {
        // CPU 全部信息挤在一行（占用 · 核数 · 负载）：分两行时整框显得碎，也白占一行高度
        const parts = [`CPU ${cpu.pct == null ? "采样中…" : cpu.pct.toFixed(1) + "%"}`];
        if (Number.isFinite(cpu.cores) && cpu.cores > 0) parts.push(`${cpu.cores} 核`);
        if (Number.isFinite(cpu.load1)) parts.push(`load ${cpu.load1}`);
        el("div", { style: "margin-top:2px" }, tip).textContent = parts.join(" · ");
      }
      if (mem) {
