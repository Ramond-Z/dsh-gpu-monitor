...
- 每 GPU 一个方块：
  ...
  - 方块上写实时功率（W）
- **每台机器的 CPU / 内存占用**：方块区右侧两条**竖向细条**（C = CPU、M = 内存），与 GPU 方块同一套视觉——**填充高度 = 占用率**、**颜色 = 同一套四级色阶**（绿 <40% < 黄 <70% < 橙 <90% < 红）、**条下横排百分数**；两条随方块区高度拉伸，悬停细条两行同款格式（**百分比（已用/总量）**）：CPU 为 `占用%（load / 核数）`，内存为 `占用%（已用G / 总量G）`。面板不够宽时细条整块落到方块下方（方块每行数量不变）。数据取自 `/proc/stat`、`/proc/meminfo`、`/proc/loadavg`（**仅 Linux**；非 Linux 机器自动不显示细条）
